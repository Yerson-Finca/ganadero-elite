...
} catch(e) { console.log('Sin datos guardados'); }

function save() {
    try { localStorage.setItem('ganadero_elite_v2', JSON.stringify(DB)); } catch(e) {}
}
...
function getCatalogoCompleto() {
    return CATALOGO_SANIDAD.concat(DB.suplementos);
}
...
function renderLote() {
    ...
    for (var i = 0; i < DB.animales.length; i++) {
        ...
        for (var j = 0; j < DB.aplicaciones.length; j++) {
            if (DB.aplicaciones[j].animalId === DB.animales[i].id) costoAnim += (DB.aplicaciones[j].costo || 0);
        }
        csTotal += costoAnim;
    }

    for (var i = 0; i < DB.animales.length; i++) {
        var a = DB.animales[i], cp = a.historial[a.historial.length - 1].peso;
        totalKg += cp;
        var d = getDiet(cp);
...
function addAnimal() {
    ...
    save(); document.getElementById('newN').value = ''; document.getElementById('newW').value = '';
    toggleAdd(); renderLote(); showToast('✅ ' + n + ' registrado');
}
...
function closeProfile() { var btn = document.getElementById('btnBackFloat'); if (btn) btn.remove(); document.getElementById('v-perfil').classList.add('hidden'); document.getElementById('nav').style.display = 'flex'; renderLote(); }
...
function openAplicarProducto(animalId) {
    ...
    for (var i = 0; i < catalogo.length; i++) {
        prodOptions += '<option value="' + catalogo[i].id + '">' + catalogo[i].nombre + '</option>';
    }
...
function renderPrecios() {
    ...
    for (var i = 0; i < ALIMENTOS.length; i++) {
        html += '<div style="display:flex;align-items:center;gap:8px;padding:11px 0;border-bottom:1px solid rgba(255,255,255,.03);"><i class="fa-solid ' + IC_ALIMENTOS[ALIMENTOS[i]] + '" style="width:18px;"></i><span style="flex:1;font-size:.78rem;">' + NM_ALIMENTOS[ALIMENTOS[i]] + '</span><span style="font-size:.7rem;color:var(--muted);">$</span><input id="pr-' + ALIMENTOS[i] + '" type="number" value="' + (DB.precios[ALIMENTOS[i]] || 0) + '" style="width:85px;text-align:right;padding:8px 10px;"></div>';
    }
...
function agregarCompraSanidad(prodId) {
    ...
    save(); mlEl.value = ''; costoEl.value = '';
    showToast('✅ Compra registrada ($' + fm(costo / ml) + '/ml)'); renderSanidad();
}
...
function eliminarSuplementoConfig(id) {
    if (confirm('⚠️ ¿Eliminar este suplemento?')) { DB.suplementos = DB.suplementos.filter(function(s) { return s.id !== id; }); save(); renderConfig(); showToast('✅ Suplemento eliminado'); }
}

function exportarDatos() { var blob = new Blob([JSON.stringify(DB, null, 2)], { type: 'application/json' }); var a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'ganadero-elite-respaldo.json'; a.click(); showToast('✅ Datos exportados'); }
function importarDatos() { var input = document.createElement('input'); input.type = 'file'; input.accept = '.json'; input.onchange = function(e) { var reader = new FileReader(); reader.onload = function(e) { try { DB = JSON.parse(e.target.result); save(); renderLote(); showToast('✅ Datos importados'); } catch(err) { alert('❌ Error al importar'); } }; reader.readAsText(e.target.files[0]); }; input.click(); }
